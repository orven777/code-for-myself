...
感谢原作者提供的脚本 https://raw.githubusercontent.com/leafxcy/JavaScript/main/jrttjsb.js 原作者20220407删库跑路
...
let validList = []
let adIdList = [26, 181, 186, 287, 188, 89, 190, 195, 210, 314, 216, 225, 308, 324, 327, 329]
...
function showUpdateMsg() {
    console.log('\n2022.04.11 16:30 更新：默认UA为安卓,DreamSeven修改\n ReadArticles() 行为异常，关闭阅读')
}
...
async function RunMultiUser() {
    ...
    //await ReadArticles() 行为异常，20220411关闭
...
async function ListArts() {
    ...
    let url = `${hostname}/api/news/feed/v64/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
...
async function ReadArtsReward() {
    ...
    let url = `${hostname}/luckycat/lite/v1/activity/done_whole_scene_task/?aid=35&update_version_code=87507&os_version=10&device_platform=android`
...
async function DailyArtsReward() {
    let caller = printCaller()
    let rndGroupId = Math.floor(Math.random()*7000000000000000000)
    let url = `${hostname}/score_task/v1/task/get_read_bonus/?aid=35&update_version_code=87507&os_version=10&device_platform=android&group_id=${rndGroupId}`
...
async function DailyPushReward() {
    ...
    let rndGroupId = Math.floor(Math.random()*7000000000000000000)
    let url = `${hostname}/score_task/v1/task/get_read_bonus/?aid=35&update_version_code=87507&os_version=10&device_platform=android&group_id=${rndGroupId}&impression_type=push`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function ReadDouble() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/activity/double_whole_scene_task/?aid=35&update_version_code=87507&os_version=10&device_platform=android`
    let body = `{}`
    let urlObject = populatePostUrl(url,body)
    await httpPost(urlObject,caller)
    let result = httpResult;
...
async function GetNewTabs() {
    let caller = printCaller()
    let url = `${hostname}/score_task/v1/user/new_tabs/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function PostInviteCode() {
    ...
    let url = `${hostname}/luckycat/lite/v1/invite/post_invite_code/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populatePostUrl(url,body)
    await httpPost(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryCoinInfo() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/user/profit_detail/?offset=0&num=100&income_type=2&aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryUserInfo(doTask) {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/task/page_data/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function SignIn() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/sign_in/action?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populatePostUrl(url)
    await httpPost(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function OpenTreasureBox() {
    let caller = printCaller()
    let timeInMS = Math.round(new Date().getTime())
    let url = `${hostname}/score_task/v1/task/open_treasure_box/?os_api=25&device_type=LYA-AL10&ssmix=a&manifest_version_code=8750&dpi=640&abflag=3&pass_through=default&cookie_data=r9I0sFSicSIPwRFv8TzScQ&act_hash=7e70351dfa59b8c82af7a377be811d19&rom_version=25&app_name=news_article_lite&ab_client=a1%2Ce1%2Cf2%2Cg2%2Cf7&version_name=8.7.5&ab_version=668907%2C3858186%2C3937398%2C1859937%2C668908%2C3937402%2C668905%2C3937366%2C668904%2C3937345%2C668906%2C3937374%2C668903%2C3937392%2C3801463%2C3520490%2C3596061%2C3958946&plugin_state=280419485511709&sa_enable=0&ac=wifi&_request_from=web&update_version_code=87507&channel=lite_huawei_64&_rticket=${timeInMS}&status_bar_height=24&cookie_base=JHxMbgU7Ww5OT8KquQLNL3EhjhgrxVIfADSHANMK_IDqxBgJmQvAA26OJcZGmZZOvvJKPomJcKkiXKYUIylmjg&dq_param=0&device_platform=android&iid=2714026057937591&version_code=875&polaris_version=1.0.5&tma_jssdk_version=2.8.0.14&cdid=99bc14e7-1c52-4f3e-a35e-5b2b7cd7b645&is_pad=0&openudid=711ca30d9d3c10b7&device_id=67558776156&resolution=1440*3020&act_token=EkH_Y0pLurayZZXqPsZVD1ovQiBht7tTWuuLw64ORdW8KPm0a5cLd76AVpRRoQbpLByAdJyJTBf6dJicEVPcJg&os_version=10&language=zh&device_brand=HUAWEI&aid=35&ab_feature=z1`
...
async function ExcitationAd(task_id) {
    let caller = printCaller()
    let timeInMS = Math.round(new Date().getTime())
    let url = `${hostname}/luckycat/lite/v1/task/done/excitation_ad?os_api=25&device_type=LYA-AL10&ssmix=a&manifest_version_code=8750&dpi=640&abflag=3&pass_through=default&cookie_data=r9I0sFSicSIPwRFv8TzScQ&act_hash=7e70351dfa59b8c82af7a377be811d19&rom_version=25&app_name=news_article_lite&ab_client=a1%2Ce1%2Cf2%2Cg2%2Cf7&version_name=8.7.5&ab_version=668907%2C3858186%2C3937398%2C1859937%2C668908%2C3937402%2C668905%2C3937366%2C668904%2C3937345%2C668906%2C3937374%2C668903%2C3937392%2C3801463%2C3520490%2C3596061%2C3958946&plugin_state=280419485511709&sa_enable=0&ac=wifi&_request_from=web&update_version_code=87507&channel=lite_huawei_64&_rticket=${timeInMS}&status_bar_height=24&cookie_base=JHxMbgU7Ww5OT8KquQLNL3EhjhgrxVIfADSHANMK_IDqxBgJmQvAA26OJcZGmZZOvvJKPomJcKkiXKYUIylmjg&dq_param=0&device_platform=android&iid=2714026057937591&version_code=875&polaris_version=1.0.5&tma_jssdk_version=2.8.0.14&cdid=99bc14e7-1c52-4f3e-a35e-5b2b7cd7b645&is_pad=0&openudid=711ca30d9d3c10b7&device_id=67558776156&resolution=1440*3020&act_token=EkH_Y0pLurayZZXqPsZVD1ovQiBht7tTWuuLw64ORdW8KPm0a5cLd76AVpRRoQbpLByAdJyJTBf6dJicEVPcJg&os_version=10&language=zh&device_brand=HUAWEI&aid=35&ab_feature=z1`
...
async function QueryWalkInfo() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/walk/page_data/?aid=35&update_version_code=87507&os_version=10&device_platform=android`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function GetWalkBonus() {
    ...
    let url = `${hostname}/luckycat/lite/v1/walk/bonus/?aid=35&update_version_code=87507&os_version=10&device_platform=android`
...
async function DoneEat() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/eat/done_eat/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populatePostUrl(url)
    await httpPost(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QuerySleepStatus() {
    ...
    let url = `${hostname}/luckycat/lite/v1/sleep/status/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function SleepStop() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/sleep/stop/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populatePostUrl(url)
    await httpPost(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function SleepDone(amount) {
    let caller = printCaller()
    let timeInMS = Math.round(new Date().getTime())
    let url = `${hostname}/luckycat/lite/v1/sleep/done_task/?os_api=25&device_type=LYA-AL10&ssmix=a&manifest_version_code=8750&dpi=240&abflag=3&pass_through=default&use_ecpm=0&rom_version=25&rit=coin&app_name=news_article_lite&ab_client=a1%2Ce1%2Cf2%2Cg2%2Cf7&version_name=8.7.5&ab_version=668907%2C3858186%2C3937398%2C1859937%2C668908%2C3937402%2C668905%2C3937366%2C668904%2C3937345%2C668906%2C3937374%2C668903%2C3937392%2C3801463%2C3520490%2C3596061%2C3958946&plugin_state=280419485511709&sa_enable=0&ac=wifi&_request_from=web&update_version_code=87507&channel=lite_huawei_64&_rticket=${timeInMS}&status_bar_height=24&dq_param=0&device_platform=android&iid=2714026057937591&version_code=875&polaris_version=1.0.5&tma_jssdk_version=2.8.0.14&is_pad=0&resolution=1440*3020&os_version=10&language=zh&device_brand=HUAWEI&aid=35&ab_feature=z1`
...
async function SleepStart() {
    let caller = printCaller()
    let url = `${hostname}/luckycat/lite/v1/sleep/start/?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populatePostUrl(url)
    await httpPost(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryFarmInfo() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/polling_info?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function EnterFarm() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/home_info?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function RewardFarmThreeGift(gift_id) {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/reward/gift?game_client_version_code=2&gift_id=${gift_id}&watch_ad=0&double=0&aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryFarmThreeGift() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/gift/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryFarmTask() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/daily_task/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function FarmWater() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/land_water?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function FarmOpenBox() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/box/open?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function FarmOpenBoxVideo() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/excitation_ad/add?excitation_ad_score_amount=134&device_id=2392172203611735&aid=35&os_version=10&update_version_code=87507`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryFarmSignStatus() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/sign_in/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function FarmSign() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/reward/sign_in?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function FarmSignDouble() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/reward/double_sign_in?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function FarmUnlock(land_id) {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_farm/land/unlock?land_id=${land_id}&aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryTreeSignStatus() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/sign_in/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function TreeSign() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/sign_in/reward?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryTreeChallenge() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/challenge/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function TreeChallengeReward() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/challenge/reward?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function TreeNutrientSign() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/nutrient/sign_in?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function RewardTreeThreeGift(task_id) {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/three_gift/reward?task_id=${task_id}&watch_ad=0&extra_ad_num=0&aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryTreeThreeGift() {
    ...
    let curTime = new Date()
    let curHour = curTime.getHours()
    let url = `${hostname}/ttgame/game_orchard/three_gift/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryTreeWaterTask() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/tasks/list?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function TreeWater() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/tree/water?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function QueryTreeStatus() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/polling_info?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
...
async function RewardTreeWaterBottle() {
    let caller = printCaller()
    let url = `${hostname}/ttgame/game_orchard/water_bottle/reward?aid=35&update_version_code=87507&device_platform=android&&device_type=LYA-AL10`
    let urlObject = populateGetUrl(url)
    await httpGet(urlObject,caller)
    let result = httpResult;
    if(!result) return
